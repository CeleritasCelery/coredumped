...
In my [last post](https://coredumped.dev/2021/10/21/building-an-emacs-lisp-vm-in-rust/) I introduced an Emacs Lisp VM I was [writing in Rust](https://github.com/CeleritasCelery/rune). My stated goal at the time was to complete a garbage collector. I think Rust has some really interesting properties that will make building garbage collectors easier and safer. Many of the techniques used in my GC are not original have been developed by other Rustaceans in previous projects.


## Why use garbage collection? {#why-use-garbage-collection}

In virtual all non-trivial programs we need some way to reuse memory. When we are done with it the memory can be freed.  But at the same time we need to make sure that no accessible memory gets freed. In Rust this is normally solved through the lifetime system. Rust tracks every allocation statically and knows when some memory is no longer in use. However this system is not flexible enough for some applications. In that case Rust gives you [Rc](https://doc.rust-lang.org/std/rc/struct.Rc.html), the reference counting cell. This tracks the number of owners to a piece of memory at runtime. Reference counting has the advantage that is relatively easy to implement and integrates seamlessly with non-rc code. However it also has two big downsides. First, it's slow[^fn:1], and second, it can't detect cyclic data (which lisp is full of) leading to memory leaks. For these reasons (and others) dynamic languages often use garbage collection (GC) to manage data.


## why is GC hard? {#why-is-gc-hard}

In his book _crafting interpreters_ Robert Nystrom has a whole [section](http://craftinginterpreters.com/garbage-collection.html#garbage-collection-bugs) dedicated to some of the tricky bugs you can have in a garbage collector. The problem lies in finding all objects that are live (accessible) in the heap. Once you have an object you know is live, it is fairly easy to trace through anything it points to and find other live data. But you need some way to find these pointers that point to other GC data, but have no data pointing to them. Problem is, these pointers are scattered across the stack or in machine registers. If you miss even one, you open yourself up to memory unsafety.


### How does Emacs solve this problem? {#how-does-emacs-solve-this-problem}

Emacs (and many C based GC implementations) solve for this by recognizing that the stack is just a block of memory[^fn:2]. If a object can't be reach from the stack, it can't be reached at all. So when garbage collection is triggered, they will run some code to dump all registers to the stack, then scan the stack for anything that "looks like" a pointer. I say looks like because we can't _actually_ know if something is pointer or just a number in range of a  pointer. There is no type information in the hardware. So anything that might be a pointer is treated as a pointer and traced. However because we aren't sure, we can't move any of the gc data. In this implementation we are building a "precise" collector that knows exactly what is a pointer and what is not. That rules out blind stack scanning.


### Let's start from the beginning {#let-s-start-from-the-beginning}

When we allocate a new object, we know that it is unaliased (nothing has a pointer to it). But as soon as we give that pointer to user code, it becomes exposed. Problem is, we need to know when it safe to call `drop` and release the memory.  In C, it is up to the user to call `free` when they are done with it. But Rust tracks the liveness of the data with the type system. The Rust rule is: there can be many immutable references to an object or one mutable reference (but not both). If you have immutable references, they get invalidated as soon as a mutable reference is used.


### affine types {#affine-types}

This key property of Rust (called affine types) is what is used in a gc library [Jospehine](https://docs.rs/josephine/latest/josephine/). We use Rust's borrow checker to ensure no references are live after we collect. All pointers into the GC heap borrowed from our allocator (called `Arena`) via a immutable reference. When we call `garbage_collect`, we take a `&mut Arena`, ensuring that all heap references are no longer accessible.
...
Seems pretty easy right? Ready to ship! Well... Not quite. If we  invalidate all references to the GC heap, we can't access the heap at all after we call `garbage_collect`.


## Rooting {#rooting}

See what we really want is to have some pointers _preserved_ across calls to `garbage_collect`. But we need to make sure the gc knows about these special pointers, or it will free the data the point to. We call these special pointers roots.
...
Thankfully, I am not the first person to think about this problem. Saoirse has a [blog post](https://without.boats/blog/shifgrethor-iii/) about some novel observations on rooting in Rust. This was implemented in his gc library [shifgrethor](https://github.com/withoutboats/shifgrethor). I will attempt to summarize this approach below.

First, is if we control when some type is created **and** when it's dropped, we deterministically know exactly when it is live. So if we use a macro like `letroot` that creates a new variable, but only returns a reference to it, we know that when we exit the scope it will be dropped in a specified order.

```rust
{
    // x will be created before y
    letroot!(x);
    letroot!(y);
    // and y will always drop before x
}
```

This behavior looks stack-like, and it fact it is. So rather then using a `HashMap` of roots, we can instead use a stack. When something is rooted, it is pushed on the stack. When it drops, it is popped from the stack. This also solves our problem of creating a unique `Token` to find our object, because when we drop we know that our item will always be the top of the root stack. So no `Token` needed.

As far as keeping our references around post garbage collection, we know that so long as our object is rooted, it will be valid. So we can keep a reference into the Gc heap until we unroot. To do this we use a macro to change our reference from borrowing from `Arena` to a borrowing from a root. So long as the root is live, our reference is valid. Even if we garbage collect.
...
This macro releases the _mutable_ borrow and reborrows the object with an _immutable_ borrow. This frees `Arena` to be used by other code while still being sound.


### Preventing escapes {#preventing-escapes}

This approach works fine for rooting a single object, but what about when we have a whole collection of objects? You might be tempted to think that would be an non-issue, but consider the problem below:
...
We take page out of the `pin` API, since we are trying to solve a similar problem. If you have a `Pin<P>` you know that the data point to by `P` will not move. Similarly, we create a `Root<T>` type that guarantees `T` is rooted. We use a macro to take a data structure `T` and returns a `&mut Root<T>` to it.

```rust
let root_owner: RootOwner = ...;
let arean: Arena = ...;
struct_root!(my_struct, arena);
let _: &Root<Vec<Object>> = my_struct;
// immutable borrow
...
Nope. Though apparently we not the only ones who need this. The std lib created a [hack](https://github.com/rust-lang/rust/pull/82834) to avoid miscompilations with aliasing mutable references that is used in [Tokio as well](https://github.com/tokio-rs/tokio/pull/3654). We could take that route (and I was really tempted to) but let's see if there is another way we could fix this.
...
[Qcell](https://docs.rs/qcell/latest/qcell/index.html) is a crate trying to solve an interesting problem; could we have a compile time `RefCell`? It makes a bunch of different cell types, each with their own set of trade off, that give you exactly that. We are going to use [LCell](https://docs.rs/qcell/latest/qcell/struct.LCell.html)[^fn:3], which is zero cost and perfect for our use case. With this type multiple cells have a shared owner that control when a cell can be borrowed mutable or immutable. To make this safe we define the following conditions:

1.  We can borrow a `Root` as immutable if we have a `&RootOwner`.
2.  We can borrow the `Root` as mutable if we have `&mut RootOwner` **and** we have a `&Arena`. This ensures that we can never call garbage collect while our mutable reference is live, because garbage collect requires a mutable borrow of `Arena`.
...
Using compile-time interior mutability makes our code more verbose then it needs to be, but that is the price we pay sometimes for correctness. I would love for Rust to get an `AliasCell` that solves this problem for everyone.
...
What I think is really cool is that the API is **safe**! You couldn't create this in `C`  or `C++`. The type system is not powerful enough. Rust enables us to have "fearless garbage collection", and no longer be scared of the "nasty bugs" that we might create. As an anecdote, I was pleasantly surprised to find that when I turned on freeing the memory in my gc, everything just worked first time; No memory leaks, no use-after-free. The API just took care of it at compile time. Miri was satisfied as well.

Overall I am pretty happy with how it turned out. That being said, there is **a lot** of unsafe code behind the scenes. I am the only person that has reviewed it, and I am not smart enough to get everything right. I created a [unsound?](https://github.com/CeleritasCelery/rune/labels/unsound%3F) Label on Github that tracks some of the code I have the least confidence in (though it all needs review). If you are initiated in the dark arts of the [nomicon](https://doc.rust-lang.org/nomicon/), I would love for you to [prove me wrong](https://github.com/CeleritasCelery/rune/issues?q=is%3Aissue+label%3Aunsound%3F+).

I am going to continue work on bootstrapping more elisp files to eventually bootstrap the elisp byte compiler and use that to test my VM. I was forced to take break from that effort and implement the garbage collector because I kept using too much memory! And implementing the garbage collector was a much bigger rabbit hole then I could have ever expected. Hopefully this will help move the community forward on the quest for a Rust GC.
...
[^fn:1]: Why is reference counting slow? There is a lot that goes into it, but it boils down to three main issues:
    ...
    With all of these issues, there are techniques to try and mitigate them and get some performance back. But even a naive GC can beat a well optimized RC implementation. And optimized GC (like JVM or V8) will always outclass reference counting. See [this SO post](https://softwareengineering.stackexchange.com/questions/30254/why-garbage-collection-if-smart-pointers-are-there) and [follow up post](https://web.archive.org/web/20200325094430/http://flyingfrogblog.blogspot.com/2010/12/why-gc-when-you-have-reference-counted.html) for more.
